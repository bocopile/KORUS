...
[Go 조건] goroutine+WaitGroup, context.WithTimeout(30s), 재시도 3회,
          slog 로깅, golang.org/x/time/rate
...
[출력 - JSON] (각 AI 동일 스키마)
{
  ...
  "fundamental_view": {"signal": "BUY | SELL | HOLD", "reason": "펀더멘털/공시/실적 관점 근거"},
  "news_view":        {"signal": "BUY | SELL | HOLD", "reason": "뉴스/매크로/센티먼트 관점 근거"},
  "risk_review":      {"signal": "BUY | SELL | HOLD", "reason": "리스크/손절 관점 근거"},
...
[Rate Limit]
- KIS REST API: 초당 2건 (golang.org/x/time/rate). config.yaml `kis_tps` 참조. 주문용(3초)/조회용(0.5초) 인스턴스 분리.
...
type EventSignal struct {
    ...
    ConsensusID     int64     `json:"consensus_id"`       // Audit Log FK: signals → ai_consensus 연결 필수
