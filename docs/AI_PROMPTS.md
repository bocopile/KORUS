...
> 용도: Claude(단일 벤더)에 입력 시 해당 단계 블록만 발췌 사용
> ※ v1.5: 멀티벤더(Gemini/ChatGPT) 폐기. 합의 태스크는 동일 프롬프트를 **관점 렌즈형 Claude 서브에이전트**(펀더멘털·뉴스·리스크)에 병렬 전송. (ARCHITECTURE 3-4)

---

## 7. 단계별 프롬프트 (LLM 입력용)

> Claude에 입력할 때 해당 단계 블록만 발췌하여 사용.
> 서브에이전트 합의가 필요한 태스크(7-3 등)는 동일 프롬프트를 3개 관점 서브에이전트(펀더멘털·뉴스·리스크)에 병렬 전송한다.
...
| 7-0-4 브리핑 | 0-1 + 0-2 + 0-3 + 0-6 | Slack 마크다운 | DB(briefings) + .md 파일 | skip + 알림 |
| 7-0-6 경제학습 | 0-1 + 0-2 + 개념DB | 학습카드 JSON | DB(econ_cards) + 브리핑 주입 | skip (브리핑은 정상 발송) |
...
| 7-3 통합분석 | 0-2 + 0-3 + 7-1 + 7-2 | 합의 JSON + candidates[] | DB(ai_responses, ai_consensus) | AI fallback (12장) |
...
| 7-0-6 경제학습 | 학습카드 1개 + 복습큐 갱신 + 브리핑 주입 | 진도/복습 주기 테스트, AI_HALT 시 skip 테스트 | 생성 실패 시 학습 섹션 생략(브리핑은 발송) |
...
| 7-3 통합분석 | 3관점 서브에이전트 응답 + 합의 JSON + 개별 원본 DB 저장 | 2/3 합의 테스트, 전원 불일치→HOLD 테스트 | 관점 1개 장애 시 2개 합의 |
...
- AI JSON 스키마 검증: 디코딩 성공 후 필수 필드(signal, confidence_score, ticker) 존재+타입 검증. 누락 시 해당 AI 결과 무효.
...
[출력 - JSON 배열, 최대 10개]
{
  "news_id": "UUID or hash",
  "category": "카테고리",
  "headline": "헤드라인 (30자 이내)",
  "summary": "핵심 2~3줄",
  "market_impact": "긍정 | 부정 | 중립",
  "affected_sectors": ["반도체", "금융"],
  "tickers": ["005930", "000660"],    -- 직접 관련 종목 (없으면 [] → 섹터 참고용, 직접 주문 신호 생성 금지)
  "event_type": "실적 | 공시 | 매크로 | 테마 | 지정학 | 기타",
...
```
[역할] AI 모닝 브리핑 어시스턴트
[방식] 단일 Claude (브리핑은 관점 합의 불필요, 3-4 참조)
[Input from] 7-0-1(거시지표 JSON), 7-0-2(뉴스 JSON), 7-0-3(캘린더 JSON), 7-0-6(경제 학습카드 JSON, 선택)
...
*오늘의 경제 공부*
- 장단기 금리차 역전: 10년물−2년물 금리가 마이너스인 상태 — 오늘 연결: -0.37%p 지속, 침체 신호로 자주 인용됨
...
**0-6. 경제 기초공부 학습카드 (econ 모듈)**

```
[역할] 경제·투자 개념 튜터 (initial 단계는 입문자 대상)
[모듈] internal/econ — 0단계 보강. 독립 모듈, 브리핑(0-4)에 주입.
[방식] 단일 Claude (관점 합의 불필요). AI_HALT(12-1) 시 skip → 브리핑은 학습 섹션만 생략하고 정상 발송.
[Input from] 7-0-1(거시지표), 7-0-2(뉴스), econ 개념DB(진도/복습 이력)
[Output to] 7-0-4(브리핑 주입), DB(econ_cards)
[주기] 평일 매일 아침, 브리핑 직전 1회.

[목적] 매일 1개 핵심 개념을 "그날의 시장 맥락"과 연결해 학습. 스페이스드 리피티션(복습 큐)으로 정착.

[선정 로직 — 결정론(코드) + Claude(설명)]
- 코드: 진도(다음 미학습 개념) 또는 복습 큐(이전 카드 1·3·7·30일 주기 재노출) 중 오늘 카드 1개 선택.
- 코드: 오늘 뉴스/지표에서 등장한 용어가 미학습 목록에 있으면 우선순위 ↑ (맥락 연결성).
- Claude: 선택된 term을 오늘 데이터에 연결해 카드 본문/예시/퀴즈 생성.
  ※ '무엇을 가르칠지'(선정)는 코드, '어떻게 설명할지'는 Claude.

[난이도 단계] L1 기초(금리/인플레/환율) → L2 시장(밸류에이션/수급/섹터) → L3 전략(리밸런싱/리스크관리)
- 진도는 L1부터 순차. 사용자가 'understood' 표시한 카드는 복습 큐로 이동.

[출력 - JSON]
{
  "date": "YYYY-MM-DD",
  "card_id": "econ-0042",
  "term": "장단기 금리차 역전 (Yield Curve Inversion)",
  "level": "L1",
  "one_line": "장기금리가 단기금리보다 낮아진 비정상 상태",
  "why_today": "오늘 10년-2년 -0.37%p, 7개월째 역전 지속 → 침체 우려 근거로 인용",
  "detail": "정상적으로는 장기금리 > 단기금리. 역전되면 시장이 향후 금리 인하(=경기 둔화)를 예상한다는 신호로 해석.",
  "example": "2026-03 현재 미 10년물 4.1% < 2년물 4.5% → 역전.",
  "quiz": {"q": "금리차 역전은 보통 무엇의 선행 신호로 인용되는가?", "a": "경기 침체"},
  "related_terms": ["기준금리", "DXY", "연착륙"],
  "is_review": false
}

[브리핑 주입 형식] 0-4의 "*오늘의 경제 공부*" 섹션에 term + one_line + why_today만 1~2줄로 노출.
[조건] 카드 본문 5줄 이내, 입문자 눈높이, 틀린 단정 금지(불확실 시 "일반적으로"). 생성 실패 시 학습 섹션 생략(브리핑 발송).

[에러 시] econ 생성 실패/타임아웃 → 학습 섹션 없이 브리핑 진행(필수 아님). AI_HALT → skip.
```
...
[Go 조건] WaitGroup 병렬, golang.org/x/time/rate(초당2회), 캐시 fallback(5분 TTL),
...
### 7-3. 한미 통합 분석 (Claude 서브에이전트 합의)

```
[역할] 글로벌 매크로 분석가 + 한국 주식 트레이더
[방식] 3관점 서브에이전트 합의 (3-4 참조) — 펀더멘털·뉴스·리스크 렌즈의 Claude 서브에이전트에 동일 프롬프트 병렬 전송
[Input from] 7-2(USMarketData JSON), 7-1(KRXSignal JSON[]), 7-0-2(뉴스 JSON[]), 7-0-3(캘린더 JSON)
[Output to] 7-4(기술분석 입력), DB(ai_responses, ai_consensus)
※ signals 테이블 저장은 7-4 전담. 7-3은 candidates[]만 출력하고 signals에 직접 저장하지 않음.
[에러 시] 관점 1개 실패 → 나머지 2개 합의(2/2 일치 시 0.70). 2개 실패 → confidence 0.55 → skip(보유 청산 시그널만 예외). 3개 실패 → AI_HALT(12-1) + 긴급 알림. (3-4 장애 재계산과 동일)

[환각 방지 제약 — 필수]
- 모든 수치는 제공된 Input JSON 데이터 내에서만 인용할 것.
- 추측성 데이터와 팩트를 엄격히 구분: 추측에는 반드시 "추정" 또는 "_est" 표기.
- Input에 없는 종목코드, 가격, 지표를 생성하지 말 것.
- 불확실하면 confidence_score 0.3 이하 + "판단 보류"로 명시.
...
[서브에이전트 공통 프롬프트]
3개 관점 서브에이전트(펀더멘털·뉴스·리스크)에 동일한 데이터를 전송하되, 각 서브에이전트에는
자신의 관점(agent_lens)을 강조하라는 렌즈 지시를 덧붙인다. 출력 스키마는 동일하게 강제하여 비교 가능하게 함.

[출력 - JSON] (각 관점 서브에이전트 동일 스키마)
{
  "date": "YYYY-MM-DD",
  "agent_lens": "fundamental | news | risk",
  ...
  "watch_list": ["005930"],
  "avoid_list": ["종목코드"],
  "candidates": [{"ticker": "005930", "suggested_tier": 2, "reason": "진입 후보 근거"}],
  "key_levels": {"kospi_support": float, "kospi_resistance": float},
  "confidence_score": float64,
  "lens_view": {"signal": "BUY | SELL | HOLD", "reason": "이 서브에이전트(agent_lens) 관점의 핵심 근거"},
  "stale_warning": "stale 항목 명시"
}
※ 각 서브에이전트는 자신의 lens_view 1개만 책임진다. 3개 서브에이전트의 lens_view.signal이 합의 투표 대상.

[합의 처리]
1. 3개 관점 서브에이전트 응답 수신 (각각 타임아웃 30초)
2. lens_view.signal 다수결(2/3) → 최종 방향. tier1_analysis.action, tier2_analysis.stance, tier3_stance 등도 다수결.
3. focus_sectors: 2개 이상 관점이 언급한 섹터만 채택
4. watch_list: 합집합 (하나라도 추천하면 포함)
5. avoid_list: 합집합 (하나라도 경고하면 포함)
6. 리스크 관점이 SELL/HOLD → 신규 BUY는 HARD_BLOCK (리스크 HARD veto, 3-4). 청산/손절은 예외
7. 합의 결과 + 개별 관점 원본 응답 모두 DB 저장 (ai_responses.agent_lens)

[규칙]
- 근거 부족 시 confidence_score 0.3 이하 + 판단 보류
- stale 데이터 별도 경고
- JSON만 출력
...
### 7-3-1. 포트폴리오 구성 — 목표비중 후보 제안 (Tier1 코어)

```
[역할] 포트폴리오 매니저 (전략적 자산배분)
[모듈] internal/portfolio (구성기). 전략 근거는 STRATEGY 6-4.
[방식] 3관점 서브에이전트 합의(majority). AI는 '후보 제안'까지, 확정은 사람/정책 승인.
[Input from] 7-3(통합분석: candidates[], focus_sectors, risk_factors), 현재 보유 스냅샷, 직전 ApprovedTargetSnapshot
[Output to] 운영자 승인 큐 → 승인 시 ApprovedTargetSnapshot → 리밸런싱 엔진(3-8). DB(ai_responses, ai_consensus)
[주기] 주간 또는 분기 리밸런싱/유니버스 이벤트 시. 장중 아님(긴급성 없음).
[적용 범위] **Tier1 코어(+ETF)만.** Tier2/3은 시그널 기반이라 목표비중 산정 대상 아님.

[작업] Tier1 유니버스의 종목별 목표비중 wᵢ 후보를 제안하라.
- 편입/제외 후보와 근거(공시/실적/밸류/수급/리스크).
- 가중 조정 방향(예: 반도체 ↑, 방어주 ↓)과 근거.
- 절대 비중 수치는 '제안값'이며, 제약 위반 여부는 코드(target.go)가 최종 검증.

[제약 — 위반 시 후보 거부 (STRATEGY 6-4 / 6-2)]
- Σwᵢ = MAX_TIER1_PCT(0.40), 단일 wᵢ ≤ MAX_SINGLE_PCT(0.10)
- 종목 목표 평가액 < MIN_TRADE_VALUE 고려분은 제외 후보로 표기
- 분기 최소 보유(편입 후 1분기) 위반 교체 금지
...
[출력 - JSON]
{
  ...
  "tier": 1,
  "proposed_items": [
    {"ticker": "005930", "name": "삼성전자", "target_weight": 0.12, "action": "유지|증액|감액|편입|제외", "reason": "근거"}
  ],
  "universe_changes": [{"ticker": "...", "change": "편입|제외", "reason": "..."}],
  "constraint_check": {"sum_ok": true, "single_max_ok": true, "notes": "..."},
  "confidence_score": float64,
  "lens_view": {"signal": "...", "reason": "..."},
  "needs_human_approval": true
}

[규칙]
- target_weight 합은 0.40에 근사(±0.1%p). 위반 시 스스로 조정하거나 needs_human_approval 유지.
- 수량·주문·체결 수치는 산출하지 말 것(결정론 엔진 담당).
- AI_HALT(12-1) 중에는 신규 후보 제안 금지 → 직전 ApprovedTargetSnapshot 유지.
- JSON만 출력.
```
...
[Input from] 7-3(통합분석 JSON — focus_sectors, watch_list, candidates[]{ticker, suggested_tier}), KIS API(일봉/분봉 데이터)
             ※ candidates[]의 suggested_tier를 TechnicalSignal.Tier 초기값으로 사용. watch_list 종목은 Tier 미지정 시 7-3 분석 맥락 기준 추정.
...
[Rate Limit]
- KIS REST API: 초당 2건 (golang.org/x/time/rate). config.yaml `kis.tps` 참조. 주문용(3초)/조회용(0.5초) 인스턴스 분리.
...
재연결: exponential backoff (1s → 60s max)
  ※ 접속키 재발급 필요 시: 마지막 발급 시각 기준 1분 미만이면 대기 후 발급 (403 무한루프 방지)
  ※ 접속키 유효기간 내 재연결은 재발급 없이 기존 접속키 재사용
...
4. API 장애: 3회 연속 실패 → 매매 중단, 5분 후 재시도, 손절은 별도 5회 재시도
   Circuit Breaker (sony/gobreaker): CLOSED → OPEN → HALF-OPEN → CLOSED
   설정값: max_requests=1, interval=0, timeout=300s(5분), trip_threshold=3(연속실패)
   ReadyToTrip: ConsecutiveFailures >= 3 시 OPEN 전환
```
...
- SOFT_WARN:  포지션 크기 50% 축소 후 실행 + 경고 알림 발송. (추후 수동 승인 전환 가능)
- PASS:       통과.

※ 아래 임계값의 정의 원본은 6-2(리스크 상수). Guard는 해당 값을 참조만 합니다.

[Layer 1 — 시장 환경 Guard] (market.go)
HARD  TRADING_START 전 신규 매수/매도 차단
HARD  TRADING_END(14:50) 이후 신규 매수 차단 (매도/손절은 허용 — 거래 시간 표 참조)
HARD  반장일 12:20 이후 신규 매수 차단
...
※ 손절(STOP_LOSS)/익절(TAKE_PROFIT) 시그널은 아래 규칙 중 "미체결 주문 존재",
  "당일 진입 횟수 초과" 적용 제외. 주문 가격 괴리, 중복 주문(idempotency)만 적용.
HARD  중복 주문 (idempotency key: 종목+날짜+시그널+시퀀스)
HARD  미체결 주문 존재 시 동일 종목 신규 주문 차단  ← 손절/익절 제외
HARD  주문 가격 괴리 PRICE_DIVERGE 초과 (현재가 대비)
HARD  동일 Tier 내 당일 신규 진입 max_daily_entries_per_tier 초과  ← 손절/익절 제외
...
[거래 시간 구간별 허용 행위]
구간              신규매수  신규매도  손절/익절  미체결취소
─────────────────────────────────────────────────────
09:05~14:50      ✅         ✅         ✅          ✅       ← 정상 매매
14:50~15:20      ❌         ✅         ✅          ✅       ← 청산 전용 (TRADING_END 이후)
15:20~15:30      ❌         ❌         ✅          ✅       ← 동시호가 구간 (취소/손절만)
15:30~           ❌         ❌         ❌          ❌       ← 장 마감
※ Layer 1 TRADING_END(14:50)는 신규 매수 차단. Layer 4 15:20 규칙은 매수 선차단 추가.
...
func (e *GuardEngine) Validate(signal Signal) (bool, []GuardResult) {
    ...
    // SOFT_WARN → 포지션 50% 축소 후 실행 + 경고 알림 발송
...
2. 신뢰도 점수 산출
   - 뉴스 관점 서브에이전트 감성 분석 (7-3 뉴스 관점 활용, v1.5: 멀티벤더 평균 아님)
   - confidence_score < 0.6 → 시그널 생성 skip
   - 단일 매체 단독 보도 → confidence_score -0.1 (미확인 리스크)
...
5. 제약 사항
   - 장 종료 1시간 이내(13:50 이후) 이벤트 시그널 → 익일 처리 (당일 진입 금지)
     ※ 14:50 TRADING_END보다 1시간 앞서 마감하는 이유: 장 마감 구간 변동성 증가 + 포지션 정리 시간 확보
...
type EventSignal struct {
    ...
    Tier            int       `json:"tier"`
    Signal          string    `json:"signal"`             // BUY | SELL | HOLD
    ConfidenceScore float64   `json:"confidence_score"`   // 0.0~1.0 (confidence_score 표준 필드명 통일)
    ConsensusID     sql.NullInt64 `json:"consensus_id"`   // Audit Log FK: signals → ai_consensus. AI 합의 기반이면 연결, 순수 기술/이벤트 시그널은 NULL 허용(5-3 signals.consensus_id와 동일)
